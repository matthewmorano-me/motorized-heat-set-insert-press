'''
Spring force-stop test (force-driven descent termination).

Ram attachment on the load cell descends fast toward the spring fixture, slows
on contact, and continues pressing until the force reading hits a high
threshold indicating the fixture is bottomed out.
...
Cycle:
  ...
  4. Descend at APPROACH_SPEED_MMPS until force >= CONTACT_FORCE_THRESHOLD
  5. Slow to PRESS_SPEED_MMPS and continue until force >= BOTTOM_OUT_FORCE
  6. Hold position; press GP20 to retract
  7. Retract to top

Force values throughout are raw tared HX711 counts (no scaling).

Safety:
  - Bottom limit switch is polled every loop iteration
...
APPROACH_SPEED_MMPS = 4.0         # fast descent until contact
PRESS_SPEED_MMPS    = 2.0         # slow descent after contact

# ---------------------------------------------------------------------------
# Force thresholds — raw tared HX711 counts (gain=128).
# Tune by observing the first descent: contact should trip just as the ram
# touches the fixture; bottom-out should trip when the spring stack is fully
# compressed.
# ---------------------------------------------------------------------------
CONTACT_FORCE_THRESHOLD = 10000     # raw counts — first touch
BOTTOM_OUT_FORCE        = 500000     # raw counts — fixture bottomed out
...
LOG_FILE = _here + '/spring_force_stop_log.csv'
...
# ---------------------------------------------------------------------------
def read_force():
    # Returns raw tared HX711 counts.
    while True:
        val = hx.read()
...
# ---------------------------------------------------------------------------
# Two-phase descent: fast approach, slow press, stop on bottom-out force.
# ---------------------------------------------------------------------------
def descend_until_bottom(t0):
    print(f"Approach at {APPROACH_SPEED_MMPS} mm/s. "
          f"Contact threshold={CONTACT_FORCE_THRESHOLD}, "
          f"bottom-out threshold={BOTTOM_OUT_FORCE}.")
    ...
    try:
        while True:
            if lim_bot.value() == 1:
                print("Bottom limit hit — stopping descent.")
                break
            ...
            if not contact_made and f >= CONTACT_FORCE_THRESHOLD:
                contact_made = True
                start_stepper(PRESS_SPEED_MMPS)
                print(f"Contact detected  t={t/1000:.2f}s  force={f}  "
                      f"-> slowing to {PRESS_SPEED_MMPS} mm/s")

            if contact_made and f >= BOTTOM_OUT_FORCE:
                print(f"Bottom-out reached  t={t/1000:.2f}s  force={f}")
                break
    finally:
        stop_stepper()
...
log_f.write('time_ms,force_counts\n')
...
try:
    ...
    descend_until_bottom(t0)

    print("\nHolding position. Press GP20 to retract.")
    while stop_btn.value() == 1:
        time.sleep_ms(20)
    while stop_btn.value() == 0:
        time.sleep_ms(20)
